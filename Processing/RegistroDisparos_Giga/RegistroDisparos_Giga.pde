...
void dibujarDiana() {
  ...
  legDY += 17;
  int _cbEscW = 12, _cbEscH = 12;
  dianaCbEscX = legX;
  dianaCbEscY = legDY - _cbEscH / 2;
  noFill(); stroke(dianaEscalaReal ? color(255, 220, 50) : COL_BORDE); strokeWeight(1);
  rect(dianaCbEscX, dianaCbEscY, _cbEscW, _cbEscH, 2);
  if (dianaEscalaReal) { fill(color(255, 220, 50)); noStroke(); rect(dianaCbEscX+2, dianaCbEscY+2, _cbEscW-4, _cbEscH-4, 1); }
  fill(dianaEscalaReal ? color(255, 220, 50) : COL_TEXT);
  textFont(fontUI); textSize(12); textAlign(LEFT, CENTER);
  text("Escala real  (" + nf(DIANA_TAMANO_MM, 1, 0) + "\u00d7" + nf(DIANA_TAMANO_MM, 1, 0)
       + " mm = diana real, los puntos pueden salirse)", dianaCbEscX + _cbEscW + 4, legDY);
}
