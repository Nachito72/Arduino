...
void dibujarDiana() {
  ...
  float tRef  = tShot - DIANA_PRE_SEG;          // referencia del centro (inicio tramo azul)
  float tPre  = tRef  - DIANA_YELLOW_SEG;       // inicio de la ventana extendida (tramo amarillo)
  ...
  // Escala fija: ángulo real que subtiende la mitad de la diana a la distancia configurada.
  // Así 1 píxel = siempre el mismo ángulo independientemente del movimiento del tirador.
  float dianaHalfDeg = (float)(Math.atan(DIANA_TAMANO_MM / 2.0 / distanciaDiana) * 180.0 / Math.PI);
  float maxDev = max(dianaHalfDeg * 1.25f, 0.02f); // 25 % de margen más allá del borde
  ...
  text("DIANA  —  Trayectoria del disparo  ( \u2212" + nf(DIANA_YELLOW_SEG + DIANA_PRE_SEG, 1, 1) + " s \u2192 \u2212" + nf(DIANA_PRE_SEG * 1000, 1, 0) + " ms \u2192 \u25cf \u2192 +" + nf(DIANA_POST_SEG * 1000, 1, 0) + " ms )", cx, gy + 8);
  ...
  text("Pre-disparo (" + nf(DIANA_YELLOW_SEG, 1, 0) + " s)", legX + 26, legDY + 1);
  legDY += 18;
